...
Scheduled jobs:
  1. Daily FSSAI/AYUSH/LM regulation scrape (6:00 AM IST = 00:30 UTC)
  2. License expiry reminder emails (7:00 AM IST = 01:30 UTC)
"""
...
def _run_daily_scrape():
    ...
    try:
        from app.services.scraper import scrape_fssai_pages, download_and_hash, classify_regulation_change
        ...
        # If critical changes found, re-check all labels
...
def _run_license_expiry_reminders():
    ...
    try:
        ...
        for lic in expiring:
            ...
            try:
                send_license_expiry_email(user, lic, days_left)
            except Exception as e:
                log.warning("[scheduler] License expiry email failed for user %s: %s", user.id, e)

        log.info("[scheduler] License expiry reminders sent.")
